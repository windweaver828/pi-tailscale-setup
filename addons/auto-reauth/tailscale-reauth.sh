...
ENV_FILE="/root/utils/.nextcloud-reauth.env"
...
if [[ ! -f "$ENV_FILE" ]]; then
  log "Missing Nextcloud reauth env file: $ENV_FILE"
  exit 1
fi
...
if tailscale "${args[@]}"; then
  ...
  # Re-apply advertised routes, exit-node setting, and performance tuning.
